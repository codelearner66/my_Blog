...
public class QrUtils {
     ...
     */
    public static void createCodeToFile(@NotNull @NotEmpty String codeContent, File codeImgFileSaveDir, String fileName) {
        try {
     ...
     */
    public static   void createCodeToOutputStream(@NotNull String codeContent, OutputStream outputStream) throws WriterException, IOException {
        String trim = codeContent.trim();
     ...
     * @throws Exception
     */
    public static String parseQRCodeByUrl(@NotNull URL url) {
        String resultStr = null;
        try {
            /**ImageIO 的 BufferedImage read(URL input) 方法用于读取网络图片文件转为内存缓冲图像
             * 同理还有：read(File input)、read(InputStream input)、、read(ImageInputStream stream)
     ...
     */
    public static BufferedImage getBufferedImage(String codeContent) throws WriterException {
    ...
    public static void main(String[] args) {
        String path = "E:\\jee-2020-092\\spring boot\\my_Blog\\Blog\\src\\main\\resources\\static\\upload\\2\\";
        File file = new File(path);
        if (!file.exists()) {
            file.mkdirs();
        }
         createCodeToFile("hello world!",file,"zx");
    }
    /**
     * 将生成的二维码 发送到前端
     * @param image 缓存区的图片
     ...
     * @throws IOException
     */
    public static void output(BufferedImage image, HttpServletResponse response) throws IOException
    {
        //创建储存图片二进制流的输出流
